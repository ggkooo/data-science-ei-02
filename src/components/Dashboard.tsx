...
export function Dashboard({ data, filename, onReset }: DashboardProps) {
  ...
  const stats = useMemo(() => calculateStatistics(values, distribution), [values, distribution]);
    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col">
      <header className="sticky top-0 z-10 bg-white border-b border-zinc-200 px-4 sm:px-6 py-3 sm:py-4">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="text-base font-semibold text-zinc-900">Análise Estatística</h1>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-5 sm:py-8 space-y-6 sm:space-y-8 w-full">
        <div className="bg-white border border-zinc-200 rounded-xl px-4 sm:px-5 py-4 flex flex-wrap items-center gap-3 sm:gap-6">
          <div>
            <p className="text-xs text-zinc-400 mb-0.5">Variável analisada</p>
          ...
          <InfoBadge label="Nº de classes (k)" value={String(distribution.numClasses)} />
          <InfoBadge
        ...
        <div>
          ...
          <div className="grid grid-cols-1 min-[420px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-9 gap-3">
            <StatCard label="Mínimo" value={fmt(stats.min)} sub={currentField.unit} />
            <StatCard label="Máximo" value={fmt(stats.max)} sub={currentField.unit} />
            <StatCard label="Média (x)" value={fmt(stats.mean)} sub={currentField.unit} accent />
            ...
            <StatCard
              ...
              accent
            />
            <StatCard label="Variância Pop. (σ2)" value={fmt(stats.variance)} sub={`${currentField.unit}2`} />
            <StatCard label="Desvio Padrão (σ)" value={fmt(stats.stdDev)} sub={currentField.unit} />
            <StatCard label="CV (%)" value={`${fmt(stats.cv)}%`} sub="coef. de variação" />
        ...
        <div className="text-center pb-8">
          <p className="text-xs text-zinc-400">
            Método de dados agrupados em classes: h = AT / raiz(n), k = AT / h,
            média por somatório fi·xi e variância populacional por (somatório fi·xi2)/n - x2
          </p>
        </div>
      </main>
